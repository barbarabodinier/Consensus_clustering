...
for (simul_study_id in "4") {
  ...
  for (simul_id in 1:n_simul) {
    ...
    tmpiqr <- apply(performances[, c(continuous_metrics), ], c(1, 2), FUN = function(x) {
  ...
  write.xlsx(as.data.frame(mytable), paste0("Tables/Simulations_consensus_", method, "/Table_selection_performances_", simul_study_id, ".xlsx"),
    rowNames = FALSE, colNames = TRUE, overwrite = TRUE
  )
  write.table(mytable, paste0("Tables/Simulations_consensus_", method, "/Table_selection_performances_", simul_study_id, ".txt"),
