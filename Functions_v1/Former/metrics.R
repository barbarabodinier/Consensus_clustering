...
#' )
#' @export
StabilityMetrics <- function(selprop, K = 100, 
                             pi_list = seq(0.6, 0.9, by = 0.01), n_cat = 3, 
                             PFER_method = "MB", PFER_thr_blocks = Inf, FDP_thr_blocks = Inf,
                             group = NULL, 
                             graph = TRUE, pk = NULL, Sequential_template = NULL, 
                             clustering=FALSE) {
  if (clustering){
    graph=TRUE
  }
      ...
      if (Sequential_template[k, block_id]) {
        ...
        for (j in 1:length(pi_list)) {
          ...
          if ((tmp_PFERs[j] <= PFER_thr_blocks[block_id]) & (tmp_FDPs[j] <= FDP_thr_blocks[block_id])) {
            ...
            if (clustering){
              tmp_loglik[j] <- ClusteringStability(coprop = stab_iter_block, pi_list = pi, K = K)
            } else {
