...
#' score <- StabilityScore(selprop, pi_list = c(0.6, 0.7, 0.8), K = 100)
#' @export
ClusteringStability=function(coprop, nc, K=100, linkage="complete"){
  if (!is.matrix(coprop)){
    n=(1+sqrt(1+8*length(coprop)))/2
  ...
  sh_clust=hclust(as.dist(1-M), method=linkage)
  N=(n*(n-1)/2)
  q=sum(M[upper.tri(M)])
  ...
  print(nc)
    # Z=cutree(sh_clust, h=1-pi)
    Z=cutree(sh_clust, k=nc)
    print(table(Z))
    pi=sh_clust$height[length(sh_clust$height)-nc+2]
    print(pi)
    
    thr=round(K * pi)
    
    p1=stats::pbinom(thr - 1, size = K, prob = q / N, lower.tail = FALSE, log.p = TRUE)
    p2=stats::pbinom(thr - 1, size = K, prob = q / N, lower.tail = TRUE, log.p = TRUE)
    print(p1)
    if ((exp(p1)+exp(p2)-1) > 1e-3){
      stop("Probabilities do not sum to one.")
    }
    # plot(pbinom(seq(0,K), size = K, prob = q / N, lower.tail = FALSE))
    
    card=table(Z)
    
    loglik=0
    for (l in 1:length(card)){
      loglik=loglik+card[l]*(card[l]-1)*p1
      for (k in 1:length(card)){
        if (k!=l){
          loglik=loglik+card[l]*log(1-prod(rep(exp(p1), card[k])))
        }
      }
    }
    S_c=-loglik
...
# ClusteringStability=function(coprop, pi_list = seq(0.6, 0.9, by = 0.01), K=100, linkage="complete"){
#   if (!is.matrix(coprop)){
#     n=(1+sqrt(1+8*length(coprop)))/2
#     M=matrix(0, nrow=n, ncol=n)
#     M[upper.tri(M)]=coprop
...
#     loglik=0
#     for (k in 1:length(card)){ 
#       loglik=loglik+card[k]*(card[k]-1)*p1
#     }
#     S_c=-loglik
#     
#     score=c(score, S_c)
