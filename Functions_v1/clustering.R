...
Clustering <- function(xdata, Lambda = NULL, nc = NULL,
  ...
  if (any(is.na(out$coprop))){
    ...
    for (i in 1:dim(out$coprop)[3]){
      out$coprop[,,i]=impute::impute.knn(out$coprop[,,i])$data
    }
  }
...
SerialClustering <- function(xdata, Lambda, nc, pi_list = seq(0.6, 0.9, by = 0.01),
  ...
  # Step 1: calibration for stable features (stability selection)
  S=matrix(NA, ncol=1, nrow=dim(bigstab_obs)[3])
  print(dim(bigstab_var))
  for (i in 1:dim(bigstab_obs)[3]){
    S[i]=StabilityRatio(selprop=bigstab_var[i,], K=K)
  }  
  ...
  # Step 2: calibration for stable clusters (consensus clustering)
  nc_full=cbind(rep(nc, nrow(Lambda)))
  metrics2=matrix(NA, ncol=1, nrow=dim(bigstab_obs)[3])
  for (i in 1:dim(bigstab_obs)[3]){
    metrics2[i]=ConsensusScore(coprop=bigstab_obs[,,i], nc = nc_full[i], K=K, linkage = linkage)
  }
